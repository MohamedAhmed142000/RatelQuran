...
class QuranVersesLocalDataSource(private val context: Context) {
    ...
    fun loadTafsirFromAssets(context: Context): List<Tafsir> {
        val json = JsonReader.readJsonFromAssets(context, "tafseer.json")
        val type = object : TypeToken<List<Tafsir>>() {}.type
        return Gson().fromJson(json, type)
    }

    fun getTafsirForAyah(surahNumber: Int, ayahNumber: Int): Tafsir? {
        val tafsirList = loadTafsirFromAssets(context)
        return tafsirList.find { it.number == surahNumber && it.aya == ayahNumber }
    }
